...
import pytest
from src.nordvpn import haversine_km, estimate_vpn_latency, find_best_vpn_for_mirror
from src.models import MirrorConfig, PingResult, NordVPNServer
...
class TestFindBestVPNForMirror:
    def test_returns_empty_without_geo(self) -> None:
        ...
        servers = [
            NordVPNServer("DE#1", "de1.nordvpn.com", "Germany", "Frankfurt", 50.1, 8.7, 20),
        ]
    ...
    def test_returns_sorted_recommendations(self) -> None:
        m = MirrorConfig(domain="stake.com", url="https://stake.com")
        r = PingResult(
            mirror=m, is_up=True, tcp_latency_ms=30.0,
            server_lat=50.0, server_lon=8.0,
        )
        servers = [
            NordVPNServer("DE#1", "de1.nordvpn.com", "Germany", "Frankfurt", 50.1, 8.7, 20),
            NordVPNServer("US#1", "us1.nordvpn.com", "USA", "New York", 40.7, -74.0, 50),
        ]
    ...
    def test_top_n_limits_results(self) -> None:
        ...
        servers = [
            NordVPNServer(f"S{i}", f"s{i}.nordvpn.com", "DE", "Berlin", 52.5, 13.4, i * 10)
            for i in range(10)
        ]
